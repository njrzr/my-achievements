...
function AchievementsList(props) {
  ...
  return (
    <div className={`fixed top-0 left-0 z-20 bg-primary w-full h-screen overflow-auto ${toggleList === true ? 'block' : 'hidden'}`}>
      ...
      { toggleList !== false ? 
        <div>
          <div style={{ backgroundImage: `url(https://njrzr-caravaggio.vercel.app/o:webp/?image=${background})`, backgroundRepeat: "no-repeat", backgroundPosition: "center", backgroundSize: "cover" }} className="relative shadow-lg bg-secondary p-4 rounded-bl-xl rounded-br-xl w-full">
                    ...
                    <div className={`${value.progressState === 'Achieved' || value.isSecret !== true || game.devices.indexOf('Xbox360') !== -1 ? 'block' : 'hidden'}`}>
                      <div className="flex flex-row">
                        { value.mediaAssets !== undefined ?
                          <img className="relative object-cover w-1/4 rounded-tl-xl rounded-br-xl" src={`https://njrzr-caravaggio.vercel.app/o:webp/q:50/?image=${value.mediaAssets[0].url}`} alt="Achievement Logo" />
                          : ''
                        }
                        <p className="text-white py-1 px-3 text-2xl font-semibold">
                          { value.rarity.currentPercentage <= 10 && <FontAwesomeIcon icon={faGem} className="text-base"/> } { value.name }
                        </p>
                      </div>

                      <p className="text-white py-1 px-3 text-xl">{ value.lockedDescription }</p>
                      <p className="text-white py-1 px-3">
