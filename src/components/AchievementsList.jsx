      ...
      { toggleList !== false && achievements.length !== 0 ?
        <div>
          <div style={{ backgroundImage: `url(https://njrzr-caravaggio.vercel.app/o:webp/q:25?image=${background})`, backgroundRepeat: "no-repeat", backgroundPosition: "center", backgroundSize: "cover" }} className="relative flex flex-col justify-center items-center font-medium bg-secondary p-4 rounded-bl-xl rounded-br-xl w-full overflow-hidden before:absolute before:bg-black before:w-full before:h-full before:top-0 before:left-0 before:bg-opacity-50 h-auto md:h-48">
            ...
            <p className="text-white font-press flex gap-2 items-center drop-shadow shadow-black text-xl">
          ...
          <div className="grid md:grid-cols-3 p-1 md:p-2 gap-1 md:gap-2 h-3/4">
            { achievements.length !== 0 ?
                achievements.achievements.map((value, index) => {
                  return <div className={`relative border rounded-xl w-full h-60 flex items-center justify-center bg-terciary overflow-hidden ${value.progressState !== 'Achieved' && value.unlocked !== true ? 'opacity-40' : 'opacity-100'}`} key={`achievement-${index}`}>
                        ...
                        <div>
                          ...
                          <div className="flex">
                            { value.mediaAssets !== undefined &&
                              <img className="relative bg-black text-white object-cover h-24 w-2/6 rounded-tl-xl rounded-br-xl" src={`https://njrzr-caravaggio.vercel.app/o:webp/q:25?image=${value.mediaAssets[0].url}`} alt={`Achievement Logo #${index}`} />
                            }
                          ...
                          <div className="absolute font-medium left-0 right-0 bottom-2 flex justify-center items-center gap-1 text-white py-1 px-3 text-sm md:text-base text-center">
                            <span className="flex items-center font-poppins justify-center text-base bg-white text-primary rounded-full w-6 h-6">
                              G
                            </span>
                    ...
                    { game.devices.indexOf('Xbox360') != -1  &&
                      <div className={`relative w-full h-full flex flex-col justify-start gap-2`}>
                        <div>
                          ...
                          <div className="flex">
                            <img className="bg-black relative object-scale-down h-24 w-2/6 rounded-tl-xl rounded-br-xl" src={`https://njrzr-caravaggio.vercel.app/o:webp/q:100?image=http://image.xboxlive.com/global/t.${numberToHex(value.titleId)}/ach/0/${numberToHex(value.imageId)}`} alt={`Achievement Logo #${index}`} />
                            <p className="text-white text-center w-4/6 py-1 px-3 text-xl md:text-2xl font-semibold flex justify-center items-center gap-2">
                              { value.rarity.currentPercentage <= 10 && <FontAwesomeIcon icon={faGem} className="text-gold" /> } { value.name }
                            </p>
                          </div>
